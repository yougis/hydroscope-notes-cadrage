...
HydroScope doit s’intégrer dans l’écosystème existant de l'Observatoire et des partenaires.
...
- **Frontend** : application web monopage (SPA) développée en **Vue.js** , **React** , selon les compétences disponibles au sein de l'équipe OEIL. L'interface est conçue pour être accessible et adaptable aux différents profils d'utilisateurs (experts, partenaires institutionnels, grand public).
- **Cartographie** : utilisation de **MapLibre GL JS** ou **OpenLayers** pour la restitution cartographique interactive. Le système supporte le tuilage vectoriel et raster (architecture tuilage) afin d'assurer des performances optimales quel que soit le volume de données affiché.
...
- **Catalogage** : les jeux de données et les indicateurs sont référencés dans un catalogue structuré, exposé via des standards **STAC** (SpatioTemporal Asset Catalog) **Intake** et **CKAN**, assurant la traçabilité, la découverte des données interapplicative et la consultation par des utilisateurs.
...
- **Cluster de traitemenent paralélisé** : les traitement des données est effectué sur un cluster **Dask** pour faciliter la production des indicateurs sur des données lourdes.
...
@todo[priority=high, section=historisation] : valider architecture cible avec l'équipe OEIL, en particulier sur les aspects technologiques et d'intégration.
