...
**Référence :** Cahier des charges HydroScope (index.qmd) + catalogue structuré `ressources/liste_indicateurs_v3.2.csv`
**Statut :** Proposition d'architecture cible (choix techniques marqués « recommandation AMOE — à valider avec l'OEIL » dans le CdC).
**Version :** 1.0

> **Principe de lecture.** Chaque bloc est décrit du **général au détaillé** : une couche/ensemble cohérent est d'abord défini, puis ses composants, son modèle de données et ses flux. Les références aux sections du cahier des charges (`§X.Y`) sont citées en regard de chaque proposition. Les schémas sont fournis en blocs Mermaid.

---

## Table des matières

1. [Synthèse exécutive & positionnement](#1-synthèse-exécutive--positionnement)
2. [Architecture cible (vue macro en 6 couches)](#2-architecture-cible-vue-macro-en-6-couches)
3. [Structure de données géographique (modèle conceptuel)](#3-structure-de-données-géographique-modèle-conceptuel)
4. [Chaîne de traitement — pipeline de données & ETL](#4-chaîne-de-traitement--pipeline-de-données--etl)
5. [Catalogue d'indicateurs (couche métier & décisionnelle)](#5-catalogue-dindicateurs-couche-métier--décisionnelle)
6. [Couche décisionnelle — analyse multicritère & versionnement](#6-couche-décisionnelle--analyse-multicritère--versionnement)
7. [Monitoring, veille & historisation](#7-monitoring-veille--historisation)
8. [Exposition & restitution (API et interfaces)](#8-exposition--restitution-api-et-interfaces)
9. [Sécurité, exigences non fonctionnelles & gouvernance](#9-sécurité-exigences-non-fonctionnelles--gouvernance)
10. [Recommandations & points de vigilance](#10-recommandations--points-de-vigilance)
11. [Annexe — tableau synthétique des indicateurs v3.2](#annexe--tableau-synthétique-des-indicateurs-v32)
