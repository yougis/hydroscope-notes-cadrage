...
**Référence :** Cahier des charges HydroScope (index.qmd) — EPICs (5_perimetre_fonctionnel.qmd), Exigences non fonctionnelles (6_Exigences_non_fonctionnelles.qmd), Vision produit (3_vision_produit_methodologie.qmd), Contexte et objectifs (1_contexte_objectif.qmd).
**Source du besoin :** spécifications brutes fournies par l'OEIL (notes d'interface), reformulées pour rester cohérentes avec le CdC.
**Version :** 1.0

> **Note de lecture.** Ce document réorganise des spécifications d'interface existantes en deux volets complémentaires : **(1) spécifications UX** (ce que l'utilisateur voit et fait, organisé par zone fonctionnelle et rattaché aux EPICs du CdC) et **(2) architecture applicative** (comment ces fonctions sont servies techniquement, rattachées aux choix d'architecture du §7). Chaque bloc cite les sections du CdC correspondantes.
