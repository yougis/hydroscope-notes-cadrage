...
## Backlog et gestion des User Stories {#sec-backlog}

Le tableau "backlog.xlsx" présente une première structuration du backlog produit d’HydroScope, organisée en EPICS et en User Stories (US). Il constitue une traduction opérationnelle des besoins fonctionnels identifiés dans le cadre du projet.
...
Le backlog initial constitue la base contractuelle du projet, notamment pour le périmètre du MVP. Les fonctionnalités associées à ce périmètre devront être réalisées dans le cadre de la préstation.
...
Le prestataire devra proposer une organisation permettant de gérer ces évolutions de manière transparente, notamment via l’outil de gestion de projet (Azure DevOps).
...
- ils seront enrichis avec des critères d’acceptation (voir l'annexe « Critères d'acceptation »), des estimations de charge et des dépendances ;
...
@todo [priority=high, section=mvp] : Vérifier avec la MOA les User Stories à MVP=FALSE dont le statut interroge (retour MD C41) et statuer avant chiffrage.
