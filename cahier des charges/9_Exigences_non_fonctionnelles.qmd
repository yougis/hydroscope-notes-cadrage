## Exigences non fonctionnelles {#sec-exigences-non-fonctionnelles}
...
- en différé (pipeline de traitement en arrière plan) pour les traitements lourds, avec des délais maîtrisés ; le suivi de ces traitements est détaillé au @sec-monitoring.
...
@todo[priority=high, section=processing]: indiquer la volumetrie pour les traitements d'indicateur sur des sources lourdes si cela est externalisé.


Le système devra permettre :

- la gestion de **10 à 30 utilisateurs simultanés** (ordre de grandeur) ;
...
- la possibilité d’interagir avec des systèmes tiers (API sécurisés ou non, webservices OGC, ESRI,  Google Earth API, Bing, OpensStreetMap, Digital Earth Pacific, serveur FTP ... ).
...
Une maquette fonctionnelle de l’interface est fourni pour permettre de faciliter le chiffrage par le prestataire.

@todo[priority=high, section=UX]: lien à intégrer vers la maquette fonctionnelle de l'interface.
...
- disponibilité cible : **≥ 98 %** (soit 7 jours off hors maintenance planifiée) ;
- plages de maintenance définies et communiquées ;
- reprise en cas d’incident dans des délais maîtrisés (cf. SLA et engagements de service au @sec-cadre-contractuel).
