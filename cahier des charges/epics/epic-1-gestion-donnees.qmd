...
HydroScope doit pouvoir se connecter à des sources de données existantes (bases de données, services web institutionnels ex.Georep et privée) et de garantir la mise à jour régulière des informations. Ces connexions, appelées côté back (ETL/scripts d'intégration), doivent être documentées.
...
Cette étape permet les traitements ultérieurs, notamment les calculs d’indicateurs et les analyses croisées.
...
Les fonctionnalités de monitoring au sein de cet EPIC visent à assurer la maîtrise des flux de données et la fiabilité des processus d’intégration. 
  - le suivi des imports et des pipelines de données (statut des traitements, succès/échec, volumétrie, durée d’exécution, fréquence des mises à jour) ;
