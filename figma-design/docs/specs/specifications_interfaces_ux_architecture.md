...
#### Échelle d'analyse : Captages ↔ Bassins versants

La colonne bascule entre deux **échelles d'analyse** (deux étages distincts, jamais mélangés) :

| | Mode Captages | Mode Bassins versants |
|---|---|---|
| **Liste** | Captages/forages | BVAEP |
| **Carte** | Points captages en vedette ; BV des captages sélectionnés surlignés | Polygones BV colorés par classe d'indicateur ; captages du BV sélectionné mis en évidence |
| **Valeurs** | Donnée brute par captage | Agrégation des captages du BV (somme pour les stock, moyenne pour les qualité) |
| **Sélection par défaut** | Quelques captages pré-sélectionnés | Tous les BV (pas de sélection forcée) |

Les **sélections sont indépendantes** d'un mode à l'autre : basculer ne perd pas la sélection de l'autre échelle.

#### Modèle de recherche : facettes (SOLR / CKAN)

- **Recherche plein texte** — filtre la liste en direct (nom du captage, commune, bassin versant, province).
- **Suggestions** — à la saisie, propositions de **communes**, de **provinces** et d'actions de sélection (presets Top 10) ; choisir une suggestion ajoute une **facette**.
- **Facettes actives** — affichées en **puces décochables** au-dessus de la liste (ex. `Commune : Koumac ✕`, `Province : Nord ✕`). Cumul possible (**sélection multiple** : plusieurs communes et/ou provinces en même temps), combinées en **ET** ; chaque puce se retire individuellement.
- **Filtres en liste déroulante** — sélecteurs `Commune ▾` et `Province ▾` en mode **ajout** : choisir une valeur l'ajoute aux facettes actives, re-choisir la même l'en retire (toggle) ; le sélecteur reste disponible pour cumuler plusieurs valeurs.
  - En mode BV, la liste de communes est limitée aux **communes intersectant le BV à ≥ 20 %** (règle interne, non affichée).
- Les **facettes filtrent la liste** : elles ne sélectionnent rien. La sélection est un **étage séparé** (cf. ci-dessous).

#### Sélection des unités (étage séparé des filtres)

- La **sélection** est ce qui est **coché** dans la liste (lignes surlignées + compteur « n sélectionnés ») ; c'est elle qui pilote la carte et les graphiques.
- Sélection / désélection / retrait **unité par unité** (clic sur la ligne ou bouton « croix »), ou bouton « Vider ».
- **Presets « Top 10 »** (à côté du tri) — **macros de sélection**, pas des facettes : `Les 10 plus exposés`, `Les 10 moins exposés`, `Les 10 plus proches`, `Les 10 plus éloignés`. Ils **cochent** les 10 unités de la liste **déjà filtrée** par les facettes ; la sélection reste ensuite modifiable à la main.
  - Un preset appliqué apparaît en **puce décochable** (`Preset : Les 10 plus exposés ✕`) : la retirer **restaure la sélection précédente** au moment de l'application. Appliquer un autre preset remplace la puce (et le point de restauration).
- **Tri de la liste** : par distance ou par nom (commune dans le futur).

#### Règles d'affichage

- La liste reflète **l'état partagé** de la session : toute modification (sélection, filtre) se répercute sur la carte et les graphiques.
- La surbrillance des résultats de recherche doit se distinguer du style des unités sélectionnées.
- Le **croisement captages ↔ BV** est mis en évidence sur la carte dans les deux modes (lien visuel entre l'échelle point et l'échelle territoire).
...
- **Fond de carte** au choix via un **sélecteur posé sur la carte** (haut-centre) : `Carto`, `Satellite` ou `Terrain` — maquette simulée (SVG, pas de tuiles hors-ligne).
- **Gestion des couches** : panneau **« Couches »** dans le volet gauche (accordéon), style MapBox/ArcGIS — lignes avec pastille couleur + icône œil `Eye`/`EyeOff`, un toggle par couche : `Bassins versants`, `Captages`, `Périmètres`, `Zones à risque` ; badge « Source : … » de l'indicateur actif.
...
- **Interaction** : zoom, déplacement, sélection, survol (informations contextuelles), superposition de couches. (Zoom décoratif en maquette pour l'instant.)
...
- **Légende** : déplacée dans le volet gauche (section « Légende » pliable), adaptée au mode Captages/BV, plutôt qu'en superposition sur la carte.
...
- Le catalogue est la porte d'entrée de la colonne droite. Les indicateurs sont organisés **en 2 familles — ENJEUX / MENACES**, puis **thèmes**, puis **groupes à objectifs communs** (cf. v3.2 : ex. « Enjeux AEP / Sécurité sanitaire », « Menaces / Infrastructures et usages »).
...
#### Recherche groupée (dropdown)

- La recherche du volet droit exploite **l'organisation Famille → Thème → Groupe** : à la frappe, une **liste déroulante groupée** apparaît sous le champ (en-têtes de famille, puis thème, puis groupe), filtrée sur code, nom, thème, groupe ou famille.
- En bas de liste, un lien **« Voir toute la liste des indicateurs → »** ouvre la vue « Indicateurs » (sidebar), catalogue complet des 38 indicateurs v3.2, elle-même groupée Famille → Thème → Groupe (accordéons) avec compteur de résultats.

#### Filtres famille / thème / groupe pilotant la liste

- La liste des indicateurs du volet droit est **pilotée par une présélection** : **tabs familles** (`ENJEUX` / `MENACES`, un seul actif), **pills thèmes** (multi-sélection parmi les thèmes de la famille active), **dropdown groupes** (« Tous les groupes » par défaut, sinon groupes du/des thèmes cochés).
- Changer de famille **réinitialise** thèmes et groupe. Les indicateurs affichés = `CATALOGUE` ∩ famille × thèmes × groupe ; le compteur « n » reflète la liste filtrée.
- La **recherche est indépendante** de ces filtres : elle interroge le catalogue complet (plein texte) et se superpose aux filtres sans se combiner avec eux.
...
2. **Consulter la fiche d'information** (métadonnées, source, méthode, dernière mise à jour, qualité) — **toujours visible, même sans droit d'ajout**, accessible par l'**icône `ⓘ` posée sur la vignette** (volet droit) ou le bouton « Fiche » (vue Indicateurs). La fiche métadonnées est attachée à la **vignette de l'indicateur**, pas aux graphiques.
...
- Chaque graphique conserve le lien vers les **métadonnées et la fraîcheur des données** ; la **fiche métadonnées elle-même est désormais portée par la vignette/catalogue**, pas par les graphiques (§3.3).
...
Trois modes complémentaires, activés par icônes sous chaque graphique (**`ChartModeSwitcher`**) :

| Mode | Comportement | Détail |
|---|---|---|
| **Répartition** | Icône « barres » | Comparaison inter-unités par défaut (barres par unité) |
| **Vue temporelle** | Icône « dynamiques temporelles » | Série temporelle de l'indicateur pour les captages compatibles (données historisées, EPIC 1/4) — **désactivée (grisée) si l'indicateur n'a pas de série historisée** |
| **Vue des changements** | Icône « variations » | Variations de l'indicateur entre une date de début et une date de fin ; s'applique à **tous les indicateurs de tous les captages de la sélection** (soumis à données historisées) |
