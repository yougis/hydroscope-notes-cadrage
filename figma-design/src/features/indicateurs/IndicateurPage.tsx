...
import { LineChart, UnitBarChart, UnitValueTable } from '@/components/charts'
  ...
  const temporalBlock = (mode: ChartViewMode, series: number[][], title: string) => {
    ...
    return (
      <div className="mt-2 flex flex-col gap-2">
        ...
        <LineChart className="min-h-24 w-full" series={series} />
  ...
  const referentielBlock = ({
    ...
    <div className="rounded-md border border-neutral-200 bg-white p-3">
      ...
      {units.length === 0 ? (
        ...
        <UnitValueTable
          data={units.map((u) => ({ label: u.label, sub: u.sub, value: u.value }))}
          unit={ind.unit}
